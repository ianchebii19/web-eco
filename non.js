...
let shirta = document.getElementsByClassName('shirta')[0]; // Added index to select the first element with class 'suita'

let shirtaItem = [
    {
        id: '105',
        name: "Fashion Men'S Button Shirt Men Casual Print Shirt Long Sleeved Shirts",
        price: 899,
        image: 'image/shirtR1.jpg'
    },
    {
        id: '106',
        name: "Fashion Men's Turtleneck polos formal Shirt",
        price: 960,
        image: 'image/shirtR2.jpg'
    },
    {
        id: '107',
        name: "Fashion MEN CASUAL/SLIM FIT SHIRTS",
        price: 1499,
        image: 'image/shirtR3.jpg'
    },
    {
        id: '108',
        name: "Fashion 2 In 1 SportsWear TracksuitFashion Mens Casual Long Sleeve Shirts-Black",
        price: 790,
        image: 'image/shirtR4.jpg'
    }
];
...
let increment = (id) => {
    let selectedItem = shirtaItem.find((x) => x.id === id); // Fixed the finding logic
...
let generateCartItems = () => {
    if (basket.length !== 0) {
        shoppingCart.innerHTML = basket.map((x) => {
            let { id, item } = x;
            let search = shirtaItem.find((y) => y.id === id) || {};
            return `
            <div class="cart-item">
                <img width="100" src="${search.image}" alt="hhhhh" />
                <div class="details">
